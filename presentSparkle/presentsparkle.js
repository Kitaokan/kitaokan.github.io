...
function preload() {
  ...
  vid = createVideo(['assets/firepresen.mp4']);
  vid.hide(); // 初期のcanvas上での表示はオフ
}
...
function createOrangeSphere() {
  ...
  orangeSphereTimer = random(9000, 46000);
  
  vid.loop(); // 球体が生成されたときに動画をループ再生
}
...
class OrangeSphere {
  ...
  show() {
    ...
    // 球体の位置に動画を描画
    image(vid, this.pos.x - this.radius / 2 -2.5, this.pos.y - this.radius / 2 -2.5, this.radius + 5, this.radius + 5);
